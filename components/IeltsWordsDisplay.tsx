...
import { Book, BookOpen, Loader2, Volume2 } from "lucide-react";
	...
	const fetchWordDefinition = async (word: string) => {
		...
		try {
			...
			if (errorMessage.includes("not found")) {
				setDefinitionError(
					`No definition found for "${word}". Try checking the spelling or search online.`,
				);
			} else if (
				...
				setDefinitionError(
					"Unable to load definition. Please try again later.",
				);
			}
